...
class AudioLanguageDetector:
    def __init__(self):
        ...
        # self.classifier = EncoderClassifier.from_hparams(
        #     source="tmp/voxlingua107", savedir="tmp/voxlingua107"
        # )
        self.language_mapping = {"en": "English", "ru": "Russian"}
        # print(self.classifier.modules.classifier)
    ...
    def extract_subtitle_segments(self, mkv_file, save_srt=False):
        """
        Extract PGS subtitles, convert to SRT, and get timestamps
        """
        print(f"\nExtracting subtitles from: {mkv_file}")
        ...
        try:
            ...
            # Find English PGS subtitle tracks
            pgs_tracks = []
            for track in mkv.tracks:
                if track.track_type == "subtitles" and track.codec == "PGS":
                    if track.language in ["eng", "en"]:
                        pgs_tracks.append(track.track_id)

            if not pgs_tracks:
                raise ValueError("No English PGS subtitle tracks found")

            # Extract first PGS track
            sup_file = os.path.join(temp_dir, "subtitles.sup")
            cmd = ["mkvextract", mkv_file, "tracks", f"{pgs_tracks[0]}:{sup_file}"]
...
def main():
    ...
    try:
        ...
        result = detector.process_mkv("./samples/mixed.mkv", cleanup=True)
